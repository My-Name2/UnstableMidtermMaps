...
def render_travel_canvas(year: int, year_data: dict, enable_acs: bool):
    ...
    with travel_tab:
        ...
        # Controls row 2: Time slider
        time_minutes = st.slider(
            "Travel time (minutes)", min_value=5, max_value=120, value=60, step=5
        )
        
        # Controls row 3: Map display options
        st.markdown("**Map Display Options**")
        show_district_labels = st.checkbox(
            "🏷️ Show district labels on map (e.g. TX-7, CA-45)", 
            value=False, 
            key="show_district_labels",
